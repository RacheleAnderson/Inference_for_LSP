...
%    repeat 100 simulations where the parameters and the covariance matrix are estimated with HATS and SCM
...
% Model parameters
a_q_true = 600; b_q_true = 0.20; c_q_true = 1000; c_r_true = 70000; noise_true = 100;
...
%%% This will take some time. One can try first with fewer simulations: 
%numReal_vec = [1 5 10 20]; % number of realizations considered
%num_sim = 20; % number of simulations for every number of realizations in numReal_vec
...
% Lower bounds:
noise_LB=0; a_q_LB=1; b_q_LB=T0; c_q_LB=0; c_r_LB=0;
% Upper bounds:
noise_UB=500; a_q_UB=2000;  b_q_UB=Tf; c_q_UB=10000; c_r_UB=100000;
...
%% 2. Example of realizations from this parametric setting
numReal_example = 3;
[X,C_true,R_true,Q_true] = lspdata_withNoise_simulation(numReal_example,a_q_true,b_q_true,c_q_true,c_r_true,noise_true,N,time_vec);
...
dist_SCM_recomp = zeros(length(numReal_vec), num_sim);
...
for indReal = 1: length(numReal_vec)
    ...
    for sim = 1: num_sim
        ...
        Q_SCM_recomp = noise_SCM + a_q_SCM * exp(-(c_q_SCM/2)*((tau_Q-b_q_SCM).^2));
        R_SCM_recomp = exp(-(c_r_SCM/8).*(tau_R).^2);  
        C_SCM_recomp = (R_SCM_recomp.*Q_SCM_recomp); 
        ...
        S2_SCM_recomp = sqrt (sum(sum ((C_SCM_recomp - C_true).^2))); % in the paper, this is LS SCM
        
        dist_HATS(indReal,sim) = S2_HATS;
        dist_SCM (indReal,sim) = S2_SCM;
        dist_SCM_recomp (indReal, sim)= S2_SCM_recomp;
...
f3 = figure()
subplot(221)
plot (numReal_vec, norm_mean_dist_SCM,'-o', numReal_vec, norm_mean_dist_SCM_recomp,'-+',numReal_vec, norm_mean_dist_HATS,'->')
